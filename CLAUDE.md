...
Restaurant Queueing System: a Next.js frontend + ASP.NET Core Web API backend for managing restaurant waitlists in real time (kiosk join-queue flow, a live "now serving" display, and an admin dashboard). Live updates are pushed via SignalR (no polling). No automated test suite exists in either project — CI only builds/lints.
...
```
No lint/test scripts are configured for the backend. On startup the app runs `db.Database.Migrate()` automatically — no manual migration step needed.
...
### CI
`.github/workflows/ci.yml` runs on push/PR to `main`: backend build, frontend lint+build, and a Docker build check for both Dockerfiles (no tests exist, so none run; no images are pushed).
